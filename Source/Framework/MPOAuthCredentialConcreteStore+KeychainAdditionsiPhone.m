...
//	NSString *itemID = [NSString stringWithFormat:@"%@.oauth.%@", [[NSBundle mainBundle] bundleIdentifier], inName];
    ...
    // Fixes a logout issues due to an inadvertent domain change https://bugz.meetup.com/show_bug.cgi?id=37983.
    if (status == errSecItemNotFound) {
        if ([securityDomain isEqualToString:@"api.meetup.com"]) {
            NSMutableDictionary *searchDict2 = [searchDictionary mutableCopy];
            searchDict2[(id)kSecAttrSecurityDomain] = @"www.meetup.com";
            status = SecItemCopyMatching((CFDictionaryRef)searchDict2, (CFTypeRef *)&attributesDictionary);
            ...
            if (status == noErr) {
                ...
                NSDictionary *keychainItemAttributeDictionary = [NSDictionary dictionaryWithObjectsAndKeys:	(id)kSecClassInternetPassword, kSecClass,
                                                                 ...
                                                                 kSecAttrAuthenticationTypeDefault, kSecAttrAuthenticationType,
                                                                 [NSNumber numberWithUnsignedLongLong:'oaut'], kSecAttrType,
                                                                 foundValue, kSecValueData, nil];
                
                SecItemAdd( (CFDictionaryRef)keychainItemAttributeDictionary, NULL);
            }
        }
    }
